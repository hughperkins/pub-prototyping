...
def run():
    ...
    print(data_samples.numpy())
    plt.scatter(range(num_data_samples), data_samples.numpy())
    plt.savefig('/tmp/data.png')
    d = Discriminator(hidden_size=hidden_size)
    g = Generator(hidden_size=hidden_size)
    params = set(d.parameters()) | set(g.parameters())
    ...
    while True:
        ...
        # data sample
        # sample_idx = int(torch.rand(batch_size, 1) * num_data_samples[0])
        # d_out = d(autograd.Variable(data_samples[sample_idx]))
        ...
        if epoch % 5000 == 0:
            ...
            X = torch.arange(x_min, x_max + interval, interval)
